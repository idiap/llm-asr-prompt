...
#   - Added bf16 training configuration support
...
def main(kwargs: DictConfig):
    ...
    if not (train_config.enable_fsdp or train_config.enable_ddp) or rank == 0:
        if log_config.use_wandb:
            ...
            wandb.init(
                dir=log_config.wandb_dir,
                entity=log_config.wandb_entity_name,
                project=log_config.wandb_project_name,
                name=log_config.wandb_exp_name,
                config=wandb_config,
            )

    model_factory = get_custom_model_factory(model_config, logger)
    model, tokenizer, processor = model_factory(train_config, model_config, **kwargs)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"{model}")

    # Convert the model to bfloat16 if fsdp and pure_bf16 is enabled
    if train_config.use_bf16:
        logger.info(f"NOT converting model to torch.bfloat16")
        logger.info(f"everything about torch.bfloat16 will be taken care by autocast")

    if (train_config.enable_fsdp or train_config.enable_ddp) and fsdp_config.pure_bf16:
        logger.info(f"converting model to torch.bfloat16")
        logger.info(f"convertinng model to torch.bfloat16")
    ...
    if train_config.enable_fsdp:
        ...
        fsdp_config.sharding_strategy = getattr(
            ShardingStrategy, fsdp_config.sharding_strategy
        )
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        ...
        lr_lambda=lambda step: (
            min(step / train_config.warmup_steps, 1)
            if step < train_config.warmup_steps
            else max(
                0.0,
                1
                - (step - train_config.warmup_steps)
