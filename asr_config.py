...
#   - Added new configuration parameters: save_checkpoint_only_at_epoch_end, freeze_projector,
#     use_bf16, prompt_num_virtual_tokens, and prompt_token
#   - Added support for "p-projector" and "p-tuning" in peft_method options
...
@dataclass
class ModelConfig:
    file: str = "model/slam_model_asr.py:model_factory"
    ...
    vq_encoder_out: bool = False
    ctc_weight: float = 0.0
    save_align_cosine_sim_matrix: pathlib.Path = field(
        default="none",
        metadata={"help": "if dir is passed, will save first 50 utt cosine sim"},
    )


@dataclass
class PeftConfig:
    peft_method: str = "lora"  # None , llama_adapter, prefix, "p-tuning", "p-projector"
    embs_init: str = "random"  # "random", "random-word", "bos"
    r: int = 104  # 8 in normal setup
    lora_alpha: int = 32
...
class TrainConfig:
    ...
    # To be populated from DataConfig (ignore)
    prompt_template: Optional[str] = None
    prompt: Optional[str] = None
    prompt_token: Optional[str] = None
    prompt_token_id: Optional[int] = None
    prompt_num_virtual_tokens: Optional[int] = 0

    use_bf16: bool = field(
        default=False,
        metadata={
            "help": "use bf16 (through amp). Note: diff from fsdp_config.pure_bf16, will override train_config.use_fp16"
        },
    )
...
class DataConfig:
    ...
    file: str = "slam_llm/datasets/speech_dataset.py:get_speech_dataset"
    train_data_path: Optional[str] = None
    val_data_path: Optional[str] = None
    prompt_embeddings_path: Optional[str] = None
    train_split: str = "train"
    test_split: str = "validation"
    prompt_template: Optional[str] = "USER: {prompt}\n ASSISTANT:"
    prompt: Optional[str] = None
    prompt_token: Optional[str] = "<p>"
    prompt_num_virtual_tokens: Optional[int] = 0
